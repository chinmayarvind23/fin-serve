...
| Airflow duplicate retry       | no duplicate side effect                                                     | immutable idempotent task         |
...
Chaos later injects replica/node failure, overload, bad version, and telemetry exporter failure with expected behavior defined first.
