...
Status: evidence DAG and initial local producer DAG implemented and tested with scoped fixtures.
...
rollout. Existing deployment updates still need stable-baseline receipt reuse; initial producer
requests reject existing deployment IDs and nonzero generations before collection.
