...
The first two-engine routing comparison used two actual vLLM processes on one physical GPU. Four 64-request cohorts completed only 21, 19, 24 and 24 requests. All other requests failed before content; Ray logs identify admission capacity rejection. Scale-down succeeded, then restart preflight failed. That session remains failed. A separate corrected session completed 256/256 measured requests on the same frozen workload, at 1.745–1.964 requests/s, with local scale-down/restart and partial-output failure recovery exercised. It showed no adaptive policy gain; exact policy-pair output agreement was 125/128, with no semantic quality estimate. See [scheduling and batching](scheduling-and-batching.md) for all four cells, lifecycle evidence and the retained driver exit-status ambiguity.
