...
The bridge currently has no prefix ownership signal, so affinity is inactive. Two processes on one GPU observe the same physical memory term. The policy tests and successful two-engine load session do not establish a multi-GPU adaptive gain. [Scheduling and batching](../../../docs/scheduling-and-batching.md) explains the score, limits and retained failed/corrected experiments.
