...
Airflow currently coordinates evidence registration, gate evaluation and the trusted deployment callback. A separate local producer DAG connects collection through managed probation and cleanup, including updates that reuse the stable baseline launch and collect fresh evidence. Its scheduler and lifecycle checks use scoped fixtures; full live GPU execution remains unfinished. The first produced GPU candidate was registered as rejected after failing its small development smoke; that smoke cannot replace the unchanged 32-case release suite.
